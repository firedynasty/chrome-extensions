...
Loops segments of any HTML5 video/audio on any page. Port of the YouTube Viewer's Loop Mix modal: `start, end, [loops]` lines (colon end = absolute, plain number = duration), generated table with per-row ▶, paste-from-clipboard button, white-noise burst at loop seams, floating chip (pause/resume + cancel) while a loop runs, playback continues after the final loop. Version-stamped singleton (`window.__lmx`) tears down stale injected DOM on upgrade.
