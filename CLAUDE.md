...
Lazy note-taking on YouTube watch pages. No popup — the toolbar icon toggles capture mode directly via an `action.onClicked` service worker (event-driven exception to the no-background-scripts convention). Two transcript sources: (1) a downloaded `transcripts/<videoId>.json` (`[{"start": secs, "text": ...}]`, written by the external `yt_notes.py` / `ytnotes` shell function) auto-loaded by video ID via `fetch(chrome.runtime.getURL(...))` — exposed through `web_accessible_resources` and picked up from disk per fetch, so no extension reload is needed for new downloads — rendered in the panel with follow-along highlight; (2) fallback interception of clicks on YouTube's own transcript segments (capture phase, so the video doesn't seek). Collected lines carry markdown timestamp links (`[H:MM:SS](...&t=Ns) text`); typed notes are stamped with the video's current playback time. A floating dialog shows compact `[n]` chips per collected line with a typed-note input and Copy+Clear/Clear/close buttons. Every append rewrites the clipboard with the full blank-line-joined list. Version-stamped singleton (`window.__ytn`).
